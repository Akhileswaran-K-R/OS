...
void enqueue(queue *q,process p){
  ...
  if(q->p[q->rear].at >= p.at){
    ...
    while(q->p[i].at >= p.at && i > -1){
      if(q->p[i].at == p.at){
        if(q->p[i].pri > p.pri || (q->p[i].pri == p.pri && q->p[i].pid > p.pid)){
          q->p[i+1] = q->p[i];
        }else{
          break;
        }
...
void findPriority(queue *q,int start,int end){
  process temp = q->p[start+1];
  int j = start+1;
  for(int i=start+2;i<end;i++){
    if(q->p[i].pri < temp.pri || (q->p[i].pri == temp.pri && (q->p[i].at < temp.at || (q->p[i].at == temp.at && q->p[i].pid < temp.pid)))){
      temp = q->p[i];
      j = i;
    }
  }
